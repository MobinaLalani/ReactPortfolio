...
interface StoreState {
  handleScan: boolean;
  setHandleScan: (handleScan: boolean) => void;
  refetch: (() => void) | null;
  ...
  logout: () => void;
  isSharedFleet: any; // یا boolean یا هرچی که هست
  SetIsSharedFleet: (newData: any) => void;
}
...
const useStore = create<StoreState>((set) => ({
  ...
  handleScan: true,
  setHandleScan: (handleScan) => set({ handleScan }),
  ...
  isSharedFleet: null,
  SetIsSharedFleet: (newData) => set({ isSharedFleet: newData }),
}));
