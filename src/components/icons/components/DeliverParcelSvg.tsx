import React from "react";

interface DeliverParcelSvgProps {
  color?: string;
  size?: number; // هم برای width هم height
  strokeWidth?: number;
}

function DeliverParcelSvg({
  color = "#111928",
  size = 22,
  strokeWidth = 1.5,
}: DeliverParcelSvgProps) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={(size * 18) / 22} // تناسب اصلی (22x18)
      viewBox="0 0 22 18"
      fill="none"
    >
      <circle cx="16" cy="15" r="2" stroke={color} strokeWidth={strokeWidth} />
      <circle cx="6" cy="15" r="2" stroke={color} strokeWidth={strokeWidth} />
      <path
        d="M4 14.9724C2.90328 14.9178 2.2191 14.7546 1.73223 14.2678C1.24536 13.7809 1.08222 13.0967 1.02755 12M8 15H14M18 14.9724C19.0967 14.9178 19.7809 14.7546 20.2678 14.2678C21 13.5355 21 12.357 21 10V8H16.3C15.5555 8 15.1832 8 14.882 7.90211C14.2731 7.70428 13.7957 7.22691 13.5979 6.61803C13.5 6.31677 13.5 5.94451 13.5 5.2C13.5 4.08323 13.5 3.52485 13.3532 3.07295C13.0564 2.15964 12.3404 1.44358 11.4271 1.14683C10.9752 1 10.4168 1 9.3 1H1"
        stroke={color}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M1 5H7"
        stroke={color}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M1 8H5"
        stroke={color}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M13.5 3H15.3212C16.7766 3 17.5042 3 18.0964 3.35371C18.6886 3.70742 19.0336 4.34811 19.7236 5.6295L21 8"
        stroke={color}
        strokeWidth={strokeWidth}
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
}

export default DeliverParcelSvg;
